...
function deviceCard(d){
 const p=d.probe||{},reason=d.reason?`<p class="reason">${escapeHTML(d.reason)}</p>`:"",history=historyAssessment(p);
...
function jobRows(jobs){
 ...
 return`<table><thead><tr><th>Drive / test</th><th>Status</th><th>Progress</th><th>Performance</th><th>Created</th><th>Actions</th></tr></thead><tbody>${jobs.map(j=>{
  ...
  let actions="";if(state.user.role!=="viewer"){if(j.status==="running"&&["surface_read","destructive_verify"].includes(j.kind))actions+=`<button data-action="pause" data-id="${j.id}">Pause</button>`;if(["paused","interrupted"].includes(j.status))actions+=`<button data-action="resume" data-id="${j.id}">Resume</button>`;if(active||j.status==="paused")actions+=`<button data-action="cancel" data-id="${j.id}">Cancel</button>`;if(reportable)actions+=`<button data-action="report" data-id="${j.id}">PDF</button>`}
	  return`<tr><td><button class="path-link" data-device-detail="${escapeHTML(j.deviceId)}">${escapeHTML(j.devicePath)} <span>OPEN ↗</span></button><br><small>${escapeHTML(statusLabel(j.kind))}</small></td><td><span class="status status-${j.status}">${escapeHTML(statusLabel(j.status))}</span>${j.error?`<br><small>${escapeHTML(j.error)}</small>`:""}</td><td><div class="progress" role="progressbar" aria-label="${escapeHTML(statusLabel(j.kind))} progress" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${((j.progress||0)*100).toFixed(1)}"><i style="width:${Math.max(0,Math.min(100,(j.progress||0)*100))}%"></i></div><small>${((j.progress||0)*100).toFixed(1)}%</small></td><td>${fmtSpeed(j.readBps)}<br><small>${j.readIops?`${j.readIops.toFixed(1)} IOPS`:""}</small></td><td>${fmtDate(j.createdAt)}</td><td><div class="actions">${actions}</div></td></tr>`
 }).join("")}</tbody></table>`}
...
function openDeviceWorkspace(id){
 ...
 renderDeviceWorkspace();$("#drive-workspace").showModal()
}
function renderDeviceWorkspace(){
 const device=state.devices.find(item=>item.id===selectedDeviceId);if(!device)return;
 const workspace=$("#drive-workspace"),savedScroll=workspace.scrollTop,focusedId=document.activeElement?.id,probe=device.probe||{},jobs=state.jobs.filter(job=>job.deviceId===device.id);
...
$("#devices").addEventListener("click",async e=>{
 const detail=e.target.closest("[data-device-detail]");if(detail){openDeviceWorkspace(detail.dataset.deviceDetail);return}
...
async function jobClick(e){const detail=e.target.closest("[data-device-detail]");if(detail){openDeviceWorkspace(detail.dataset.deviceDetail);return}const b=e.target.closest("[data-action]");if(!b)return;try{if(b.dataset.action==="report")await api(`/api/v1/jobs/${b.dataset.id}/report`,{method:"POST"});else await api(`/api/v1/jobs/${b.dataset.id}/${b.dataset.action}`,{method:"POST"});notify(`${b.dataset.action} requested`);await refreshAll()}catch(error){notify(error.message,true)}}
...
$("#drive-workspace").addEventListener("close",()=>{selectedDeviceId=null;selectedWorkspaceJob=null});
