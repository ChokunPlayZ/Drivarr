...
function deviceCard(d){
 const p=d.probe||{},reason=d.reason?`<p class="reason">${escapeHTML(d.reason)}</p>`:"",history=historyAssessment(p),firmwareTest=p.smartSelfTest,testCount=state.jobs.filter(job=>job.deviceId===d.id).length;
 const actions=state.user.role==="viewer"?"":`<button class="filled start" data-id="${escapeHTML(d.id)}">Run test</button>${testCount?`<button class="tonal full-report" data-id="${escapeHTML(d.id)}">Full report (${testCount})</button>`:""}${d.status==="quarantined"?`<button class="tonal retry" data-id="${escapeHTML(d.id)}">Retry probe</button>`:""}`;
...
function reportRows(){
 ...
 return`<table><thead><tr><th>Report</th><th>Coverage</th><th>Verdict</th><th>Created</th><th>Integrity</th><th>Download</th></tr></thead><tbody>${state.reports.map(r=>{const coverage=r.scope==="drive"?`Full drive · ${r.testCount} test${r.testCount===1?"":"s"}`:r.jobId;return`<tr><td><strong>${escapeHTML(r.id)}</strong></td><td>${escapeHTML(coverage)}</td><td><span class="status status-${r.verdict==="pass"?"completed":escapeHTML(r.verdict)}">${escapeHTML(r.verdict)}</span></td><td>${fmtDate(r.createdAt)}</td><td><button data-verify="${r.id}">Verify</button></td><td><a href="/api/v1/reports/${encodeURIComponent(r.id)}/download">PDF</a> · <a href="/api/v1/reports/${encodeURIComponent(r.id)}/checksum">SHA-256</a></td></tr>`}).join("")}</tbody></table>`}
...
$("#devices").addEventListener("click",async e=>{
 ...
 const fullReport=e.target.closest(".full-report");if(fullReport){await generateDriveReport(fullReport.dataset.id);return}
...
async function generateDriveReport(id){try{const report=await api(`/api/v1/devices/${encodeURIComponent(id)}/report`,{method:"POST"});notify(`Full PDF created with ${report.testCount} tests`);await refreshAll()}catch(error){notify(error.message,true)}}
...
$("#workspace-content").addEventListener("click",e=>{const b=e.target.closest(".full-report");if(b)generateDriveReport(b.dataset.id)});
